...
import { CartesianGrid, getNiceTickValues, Line, LineChart, XAxis, YAxis } from 'recharts'
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert'
import { ChartContainer, ChartLegend, ChartLegendContent } from '@/components/ui/chart'
import type { Relation } from '@/registry/calculator/lib/model'
import { DEFAULT_VIEWPORT, preparePlot } from '@/registry/calculator/lib/plot'
  ...
  const result = useMemo(() => {
    try {
      ...
      return { plot: { ...plot, curves: plot.curves.map((curve) => ({
        ...curve,
        data: curve.fn ? Array.from({ length: 400 }, (_, index) => {
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-2">
      <ChartContainer className="aspect-auto min-w-0 max-w-full overflow-hidden" config={config} initialDimension={{ width: 320, height: 360 }} style={{ width: '100%', height: 360 }}>
        <LineChart accessibilityLayer aria-label="Selected relations plot" margin={{ top: 12, right: 16, bottom: 20, left: 8 }}>
          <CartesianGrid />
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Button variant="outline" size="sm" onClick={() => setView(DEFAULT_VIEWPORT)}>Reset view</Button>
      </div>
    </div>
  )
}
